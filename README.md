...
Durante el desarrollo de esta fase, lo que más me marcó fue entender la importancia de la Arquitectura Hexagonal. Separar la infraestructura (como el CrearTicketRequest que modifiqué) del dominio asegura que el sistema sea escalable y fácil de mantener. Además, aplicar GitFlow de manera rigurosa me ayudó a visualizar cómo se organiza un equipo profesional: no es solo subir código, es seguir convenciones de commits y flujos de aprobación que garantizan la calidad antes de llegar a main. Superar la curva de aprendizaje de las GitHub Actions fue clave para entender el ciclo de vida de una entrega continua.